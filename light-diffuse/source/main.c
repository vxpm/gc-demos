...
static GXColor litcolors[] = {
    {0x00, 0xD0, 0xD0, 0xFF}, // Light color 1
    {0x20, 0x00, 0x00, 0xFF}, // Ambient 1
...
void setlight(
    ...
    GX_SetNumChans(1);
    GX_SetChanCtrl(
        GX_COLOR0A0, GX_ENABLE, GX_SRC_REG, GX_SRC_REG, GX_LIGHT0, GX_DF_CLAMP,
        GX_AF_NONE
    );
...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        for (u32 i = 0; i < count; i++) {
            Vertex v = mesh[i];
            GX_Position3f32(
                v.position.x / 5.0, v.position.y / 5.0, v.position.z / 5.0
            );
