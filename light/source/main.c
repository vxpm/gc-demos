...
void setlight(
    Mtx view, f32 theta, f32 phi, GXColor litcol, GXColor ambcol, GXColor matcol
) {
    guVector lpos;
    f32 _theta, _phi;
    GXLightObj lobj;
    ...
    GX_SetNumChans(1);
    GX_SetChanCtrl(
        GX_COLOR0A0, GX_ENABLE, GX_SRC_REG, GX_SRC_VTX, GX_LIGHT0, GX_DF_CLAMP,
        GX_AF_NONE
    );
...
int main(int argc, char** argv) {
    ...
    while (1) {
        // set number of rasterized color channels
        setlight(
            view, 8 + theta_rot, 20 + phi_rot, litcolors[0], litcolors[1],
            litcolors[2]
        );
        ...
        GX_Normal3f32(-1.0f, 1.0f, 1.0f);    // Top Left of the quad (top)
        ...
        phi_rot +=
            1.0; // Decrease The Rotation Variable For The Quad     ( NEW )

        theta_rot +=
            0.33; // Decrease The Rotation Variable For The Quad     ( NEW )

        rquad -=
            0.1f; // Decrease The Rotation Variable For The Quad     ( NEW )
    }
}
